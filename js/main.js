...
const showResult = (result) => {
    ...
    Xi.forEach((numberXi,index) => {
        ...
        <div class="w-4/12 md:w-2/12 grid grid-cols-3 my-1">
                            <h2 class="col-span-1 text-center self-center">${numberXi}</h2>
                            <h2 class="col-span-1 text-center self-center">${getShowFi(result.Fi, numberXi)}</h2>
                            <h2 class="col-span-1 text-center self-center">${getShowFa(result.Fa, index)}</h2>
                        </div>
                        <h2 class="w-4/12 md:w-2/12 my-1">${getShowFr(result,index, numberXi)}</h2>
                        <h2 class="w-4/12 md:w-3/12 text-center self-center my-1">${getShowFra(result.Fra, index)}</h2>
                        <h2 class="w-6/12 md:w-2/12 text-center self-center my-1">${getShowFp(result.Fr, result.Fp, index)}</h2>
                        <h2 class="w-6/12 md:w-3/12 text-center self-center my-1">${getShowFap(result.Fap, index)}</h2>
        `;
...
const showOrderNumber = () => {
    ...
    const result = getProcess(stringNumber);
    showXi(result.Xi);
    showResult(result);
}
const clickApp = (e) => {
